...
function html(tag, attributes, content) {
    ...
    function process_content(content) {
        var type = persist.get_type(content);
        try {
            if (type == 'hashmap') {
            ...
            else if (util.isArray(content) || type == 'list') {
                var scripts = "";
                for (var i=0; i<content.length; i++) {
                    scripts += process_content(content[i]);
                }
                return scripts;
            }
            else {
    ...
    //  Apply necesary code and remove script element
        ...
        function serialize_attributes(from, into) {
            try {
                for (var field in from) {
                    ...
                    if (deeper[field]) {
                        //  TODO: handle if from[field] is a reference
                        into[field] = {};
                        serialize_attributes(from[field], into[field]);
                    }
                    else {
        ...
        return '<script id="' + script_id + '" type="text/javascript">(function () {var element = $("#' + id + '");\nvar script = $("#' + script_id + '");\nelement.removeAttr("id");\n' + client_source + '})();</script>';
    }
