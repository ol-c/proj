    ...
    persist.resolve_and_execute(null, operation, function (err, res) {
        if (err) {
            ...
            if (res.value.type == 'hashmap') {
                promise.reject(new Error('Cannot resolve a reference to a container. Only values in containers may be resolved.'));
            }
            else if (res.value.type == 'function') {
                promise.reject(new Error('cannot resolve a function. You must resolve the executed function.'));
            }
            else {
