...
function instance_from_serializable(serialized) {
    ...
    for (var i=0; i<path.length; i++) {
        if (path[i].type == 'call') {
            // TODO: instance from serializable for each argument
            var argument_references = [];
            var args = {};
            for (var j=0; j<path[i].arguments.length; j++) {
                var name = 'args.a' + j;
                args[name] = instance_from_serializable(path[i].arguments[j]);
                argument_references.push(name);
            }
            //  eval with all arguments;
            eval('instance = instance[path[i].name](' + argument_references.join(', ') + ')');
        }
        else { //  tyoe is reference
            instance = instance[path[i].name];
        }
    }
    console.log('loading serialized value...', serialized, serializable_from_instance(instance));
    return instance;
}
    ...
    while (name) {
        ...
        if (args) {
            ...
            for (var i=0; i<args.length; i++) {
                persist.serializable_from_instance(args[i]);
            }
        }
        else {
            ref.type = "reference";
...
function resolve(reference) {
    ...
    for (var i=0; i<path.length; i++) {
        ...
        if (ref.type == "call") {
            var source_args = [];
            for (var i=0; i<ref.arguments.length; i++) {
                source_args.push(get_source_version(instance_from_serializable(ref.arguments[i])));
            }
