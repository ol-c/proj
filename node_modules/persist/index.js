...
function JSONprotocol(context, respond, external_requester) {
    //  TODO: implement response token to pass back
    ...
    return function (data) {
        try {
            ...
            resolve_and_execute(operation, function (error, value) {
                if (error) send(error_report(error, true, operation.reference));
...
function close_connection(connection){
    return function () {
        ...
        var watching = open_connections.get(connection);
        ...
        open_connections.delete(connection);
    }
}
...
function resolve_and_execute(operation, callback) {
    ...
    if (value) {
        ...
        if (reference.internal) {
            console.log(reference.internal)
        ...
        if (operation.type == 'source reference') {
            ...
            else serializable(value, reference, callback);
        }
            ...
            //  TODO: if reference, return referenced object
            else if (type == 'reference') {
                resolve_and_execute({
                    type : operation.type,
                    reference : value, // might need to resolve 

                }, callback);
            }
            else {
                callback('only functions and references can be accessed through references');
            }
        }
            ...
            callback(null, {
                type : 'success',
                reference : reference,
                data : 'watching this reference (not really though because its not implemented yet...)'
            });
        }
        else callback('invalid operation type: ' + operation.type);
    }
    else resolve_and_execute_external(operation, callback);
}

function resolve_and_execute_external(operation, callback) {
