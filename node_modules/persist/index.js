//  process must be started with --harmony flag
var net = require('net');

var File = require('files').File;
var profile = require('profile');
var error_report = require('./errors');
var get_time = require('./history/get_time');
var get_type = require('./meta/get_type');
var records = require('./history/records');
var history = require('./history');
var modules = require('modules');

//  used by environment module
module.exports.get_type   = get_type;


var serializable_from_instance = require('./serialization/serializable_from_instance');
var instance_from_serializable = require('./serialization/instance_from_serializable');
var scoped = require('./execution/scoped');
var containers = require('./containers');
var meta = require('./meta');
var send_to_watching = require('./protocol/send_to_watching');
var proxies = require('./meta/proxies');
var events = require('./events');
var unload = require('./containers/unload');
var load = require('./containers/load');
var create = require('./containers/create');
var exists = require('./meta/exists');
var watched_references = require('./protocol/watched_references');
var resolve_and_execute = require('./protocol/resolve_and_execute');
var host_resolver = require('./meta/host_resolver');
var JSONprotocol = require('./protocol/json_protocol');
var field_settings = require('./meta/field_settings');

var auth = require('auth');

module.exports.handleWS      = handleWS;
module.exports.handleTCP     = handleTCP;
module.exports.identify      = identify;
module.exports.create        = create;
module.exports.unload        = unload;
module.exports.load          = load;
module.exports.is            = is;
module.exports.on            = events.on;
module.exports.exists        = exists;
module.exports.history       = history;
module.exports.scoped        = scoped;
module.exports.resolve_hosts = host_resolver.set;
module.exports.resolve_and_execute = resolve_and_execute;
module.exports.instance_from_serializable = instance_from_serializable;
module.exports.serializable_from_instance = serializable_from_instance;
module.exports.freeze = field_settings.freeze;
module.exports.unfreeze = field_settings.unfreeze;
module.exports.get = get;


function get(id) {
    //  TODO: some sort of universal getting convention
    //        for persistant objects and others that can
    //        be the scope of execution
    if (proxies[id]) return proxies[id];
    else return modules[id];
}

var agent;
var root;

module.exports.set_owner = function (proxy, owner) {
    //auth.access(owner, true);
    if (meta.get(proxy).owner) {
        throw new Error('proxy already has an owner');
    }
    else {
        records.save({
            time : get_time(),
            id : identify(proxy),
            operation : 'set',
            meta : true,
            field : 'owner',
            value : serializable_from_instance(owner)
        });
        meta.get(proxy).owner = owner;
    }
}

module.exports.get_owner = function (proxy) {
    return meta.get(proxy).owner;
}

module.exports.set_agent = function (name) {
    auth.access(name, true);
    agent = name;
}

module.exports.get_agent = function () {
    return agent;
}

module.exports.set_root = function (name) {
    root = name;
}

module.exports.get_root = function () {
    return root;
}

function handleWS(connection) {
    var end = profile.start('handleWS');

    var respond = JSONprotocol(connection, connection.send);
    connection.on('message', respond);
    connection.on('close', close_connection(connection));
    connection.open = true;

    end();
}

function handleTCP(connection) {
    var end = profile.start('handleTCP');

    //  TODO: set proper requester (will be another persist instance)
    var requester = 'external tcp (persist) requester';
    var respond = JSONprotocol(connection, connection.write);
    connection.on('data', respond);
    connection.on('end', close_connection(connection));
    connection.open = true;

    end();
}

function close_connection(connection) {
    return function () {
        //  TODO: clean up watching infrastructure
        connection.open = false;
    }
}

function identify(persistant) {
    if (meta.has(persistant)) {
        var id = meta.get(persistant).id
        return id;
    }
    else if (modules.is(persistant)) {
        var id = modules.identify(persistant);
        return id;
    }
    else {
        throw new Error('cannot identify non-persistant item');
    }
}

function is(proxy) {
    return meta.has(proxy);
}
//  TODO: serve historical version of object
