...
function name_to_id(name) {
    return new Promise(function (resolve, reject) {
        resolve('1393833919196379100');
    });
}

function error_report(error, serialize) {
    var report = {
        type : 'error',
        reference : reference,
        data : error.toString()
    ...
    return function (data) {
        try {
            ...
            serializable(info, function (error, response) {
                if (error) send(error_report(error, true));
                else       send(JSON.stringify(response));
            });
        }
        catch (error) {
            send(error_report(error, true));
        }
    }
}
...
function serializable(reference, callback) {
    load(reference.id, {}, function (error, value) {
...
function host(path) {
    //  TODO retrieve process id from dynamodb
    //  if no process has the object, bring the object into this one
    return new Promise(function (resolve, reject) {
        resolve('localhost');
    });
}
