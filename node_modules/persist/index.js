    ...
    return function (data) {
        try {
            ...
            if (type == 'source reference') {
                serializable(operation, function (error, response) {
                ...
                //      if is not function, return serialized version
                //      if is function, execute and return response
            }
            else if (type == 'set'){
                //  TODO: check expectation against value
                //.       if expectation passes, perform set
            }
            else {
                send(error_report('"' + type + '" is not a valid operation type', true, operation));
            }
        }
        catch (error) {
            send(error_report(error, true, reference));
        }
    }
}
        ...
        host_resolver(reference.id, function (err, data) {
            if (err) callback('error occurred resolving host for reference');
                ...
                var client = net.connect(data.port, data.host, function () {
                    client.write(JSON.stringify(reference));
                });
                client.on('data', function (data) {
                    callback(null, JSON.parse(data));
                    client.end();
