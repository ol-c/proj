...
function JSONprotocol(context, respond, external_requester) {
    ...
    return function (data) {
        //  TODO: sometimes multiple writes are lumped into data
        //        make protocol handle this case
...
var incoming_connections = new Map();
...
function close_connection(connection){
    return function () {
        ...
        var watching = incoming_connections.get(connection);
        ...
        incoming_connections.delete(connection);
    }
}
...
function handleWS(connection) {
    ...
    incoming_connections.set(connection, []);
    ...
    if (value) {
        ...
        if (operation.type == 'source reference') {
            ...
            if (type == 'function') {
                var params = {};
                try {
                    value.apply(proxies[value_parent], [null, params, function (res) {
                        callback(null, {
                            type : 'result',
                            reference : reference,
                            result : res
            ...
            //  if reference, return referenced object
            else if (type == 'reference') {
                resolve_and_execute(context, {
                    type : operation.type,
                    reference : value,
                ...
                callback('only functions and references can be evaluated');
            }
        }
...
function resolve_and_execute_external(context, operation, callback) {
     host_resolver(operation.reference.id, function (err, data) {
        if (err) callback('error occurred resolving host for reference');
            ...
            //  TODO: add token to every operation if it does not alreadt have one
            //  do this at the earliest time possible...
...
function serializable(value, reference, callback) {
        ...
        if (type == 'hashmap') {
            ...
            callback(value + ' is not serializable');
            return;
        }
...
function send_to_watching(reference, value) {
    var hashed_ref = hash_reference(reference);
    var watching = watched_references;
    if (watching) {
...
function get_persistant_proxy(id) {
    ...
    function set_field(target, field, val, receiver) {
        ...
        if (persistant || type) {
            ...
            send_to_watching(reference, val);
        }
        else {
