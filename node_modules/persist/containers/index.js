module.exports.create = create_container;
...
var send_to_watching = require("../protocol/send_to_watching");
            ...
            if (name == 'fulfill') {
                return function () {
                    if (promise.fulfilled) {
                        throw new Error('Cannot fulfill promise, already fulfilled');
                    }
                    else if (promise.rejected) {
                        throw new Error('Cannot fulfill promise, already rejected');
                    }
                    promise.fulfilled = true;
                    fulfill_args = arguments;
                    ...
                    if (meta.get(proxy).loaded) {
                        ...
                        for (var i=0; i<promise.fulfill.length; i++) {
                            if (promise.fulfill[i]) {
                                references.resolve(
                                    references.apply(
                                        promise.fulfill[i],
                                        arguments
                ...
                return function () {
                    //  if proxy is completely loaded, then save and execute call
                    if (promise.fulfilled) {
                        throw new Error('Cannot reject promise; already fulfilled');
                    }
                    else if (promise.rejected) {
                        throw new Error('Cannot reject promise; already rejected');
                    }
                    promise.rejected = true;

                    reject_args = arguments;

                    if (meta.get(proxy).loaded) {

                        for (var i=0; i<promise.reject.length; i++) {
                            if (reject_functions[i]) {
                                references.resolve(
                                    references.apply(
                                        promise.reject[i],
                                        arguments
                ...
                return function (on_fulfill, on_reject) {
                    //  arguments can be a reference to the function
                    //  or the function itself
                    //  (from which we construct the reference)
                    var fulfill_ref;
                    var reject_ref;
                    if (get_type(on_fulfill) == 'reference') fulfill_ref = on_fulfill;
                    else if (on_fulfill) {
                        var ref_to_val;
                        try {
                            ref_to_val = reference_to_value(on_fulfill);
                        }
                        catch (err) {
                            throw new Error('first argument to then must be a reference to a function, or a value we can find a reference for');
                        }
                        fulfill_ref = instance_from_serializable(ref_to_val);
                    }
                    else {
                        throw new Error('first argument to then must be a reference to a function, or a value we can find a reference for');
                    }
                    if (get_type(on_reject) == 'reference') reject_ref = on_reject;
                    else if (on_reject) {
                        var ref_to_val
                        try {
                            ref_to_val = reference_to_value(on_reject);
                        }
                        catch (err) {
                            throw new Error('second argument to then must be a reference to a function, or a value we can find a reference for');
                        }
                        reject_ref = instance_from_serializable(ref_to_val);
                    }

                    //  TODO: add promise metadata
                    promise.fulfill.push(fulfill_ref);
                    promise.reject.push(reject_ref);

                    if (meta.get(proxy).loaded) {
                        if (promise.fulfilled && fulfill_ref) {
                            references.resolve(
                                references.apply(
                                    fulfill_ref,
                                    fulfill_args
                        ...
                        else if (promise.rejected && reject_ref) {
                            references.resolve(
                                references.apply(
                                    reject_ref,
                                    reject_args
                        ...
                        records.save({
                            ...
                            field : 'then',
                            arguments : [
                                serializable_from_instance(fulfill_ref),
                                serializable_from_instance(reject_ref)
                            ]
                        });
                        ...
                        send_to_watching(null, self_reference, {
                            type : 'promise',
                            operation : 'then',
                            fulfill : [references.serializable(fulfill_ref)],
                            reject : [references.serializable(reject_ref)]
                        });
                ...
                return function (time) {
                    //  if proxy is completely loaded, then save and execute call
                    if (promise.fulfilled) {
                        throw new Error('Cannot delay promise; already fulfilled');
                    }
                    else if (promise.rejected) {
                        throw new Error('Cannot delay promise; already rejected');
                    }
                    else if (promise.delayed) {
                        throw new Error('cannot delay promise; already delayed');
                    }
                    promise.delayed = true;
                    var date;

                    if (get_type(time) == 'date') {
                        date = time;
                    }
                    else if (get_type(time) == 'number') {
                        date = new Date(Date.now() + time);
                    }
                    else {
                        throw new Error('delay only accepts dates and numbers as arguments');
                    }
                    setTimeout(function () {
                        //  fulfill promise in future or not
                        //  TODO: proper behavior depends on loading being synchronous
                        //  since we don't want a proxy to be fulfilled twice
                        if (promise.fulfilled || promise.rejected) {}
                        else proxy.fulfill()
                    },
                    date.getTime() - Date.now());

                    if (meta.get(proxy).loaded) {
                        var args = [serializable_from_instance(date)];
                        records.save({
                            time : get_time(),
                            id : id,
                            operation : 'call',
                ...
                return function() {
                    ...
                    var reject = [];
                    for (var i=0; i<promise.fulfill.length; i++) {
                        fulfill.push(references.serializable(promise.fulfill[i]));
                        reject.push(references.serializable(promise.reject[i]));
                    }
                    return {
