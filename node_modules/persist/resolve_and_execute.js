...
function resolve_and_execute(context, operation, callback) {
    var end = profile.start('resolve_and_execute');
    ...
    if (operation.type == 'update') {
        ...
        var value = references.resolve(references.instance_from_serializable(reference), true);
        ...
        //  watch operations must be passed to the proper server 
            ...
            watch_operation(context, operation, reference, callback);
        }
...
function reference_operation(value, operation, reference, callback) {
    var end = profile.start('reference_operation');
...
function evaluate_operation(value, operation, reference, callback) {
    var end = profile.start('evaluate_operation');
...
//        set : true,
    ...
    if (valid_types[type]) {
        ...
        var valid = Object.keys(valid_types);
        var last = valid.pop();
        callback('scripts must be evaluated in the context of a ' + valid.join(', ') + ' or ' + last);
    }
    end();
}

function watch_operation(context, operation, reference, callback, remote) {
    var end = profile.start('watch_operation');

    //  only the last 2 parts of the reference are uniquely identifying
    //  TODO: debug flattening remote reference...
    ...
    if (remote) {
        ...
        references.flatten(reference, function (err, flattened) {
           if (err) {
                callback(err);
                return;
            } 

            watch(reference, flattened);
        });
    }

    function watch(reference, flattened) {
        var hashed_reference = hash_reference(flattened);
...
function resolve_and_execute_external(context, operation, callback) {
    var end = profile.start('resolve_and_execute_external');
    ...
    host_resolver.get(operation.reference[0].name, agent, function (err, data) {
        ...
        if (err) callback(err);
            ...
            if (operation.type == 'watch') {
                watch_operation(context, operation, operation.reference, function () {/*done with local work*/}, true);
                //  wait for response with properly flattened reference;
