...
function resolve_and_execute(context, operation, callback) {
    ...
    if (operation.type == 'update') {
    ...
    else if (proxies[reference[0].name]) {
        var value = proxies[reference[0].name];
        var reference = references.resolve(references.instance_from_serializable(operation.reference));

        reference.then(function (value) {
        ...
        if (reference.internal) {
            var internal = reference.internal.substring(2, reference.internal.length - 2).split('"]["');
            ...
            while (internal.length) {
                ...
                if (type == 'hashmap' || type == 'list') {
                    ...
                    resolve_and_execute(context, copy, callback);
                    return;
                }
            }
        }
...
function resolve_and_execute_external(context, operation, callback) {
    ...
    console.log('hmmm', operation.reference);
